...
import * as noise_gen from "./noise_generators";
...
function pixelsJS() {
    ...
    return {
        ...
        filter_dict: getFilterNames(),
        // Browser-only function
        // Used for pre-set filters only. 
        filterImg: (img: any, filter: any) => {
            let filterExists = checkIfFilterExists(filter);
            if (filterExists) {
                let filter_dict = getFilterNames() as any;
                let canvas_and_context = imgToCanvas(img);
                let canvas = canvas_and_context[0] as any;
                let context = canvas_and_context[1] as any;
        ...
        replaceImgElement: (canvas: any, img: any) => {
            console.log("r")
            img.parentNode.replaceChild(canvas, img);
        },

        // Browser and NodeJS compatible
        filterImgData: (user_img_data: any, filter: any) => {
            let filterExists = checkIfFilterExists(filter);
            if (filterExists) {
        ...
        adjBrightness: (img: any, BRIGHTNESS_ADJ: any) => {
            let canvas_and_context = imgToCanvas(img);
            let canvas = canvas_and_context[0] as any;
            let context = canvas_and_context[1] as any;
